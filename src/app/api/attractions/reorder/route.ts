...
import { reorderAttractions } from "@/lib/attractions/service";
...
export async function PUT(request: Request) {
  ...
  try {
    await reorderAttractions(entries);
    return Response.json({ ok: true }, { status: 200, headers: NO_STORE });
  } catch (error) {
    console.error("PUT /api/attractions/reorder failed:", error);
    return Response.json(
      { ok: false, error: "Could not reorder the attractions." },
