...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await updateAttraction(id, data);
    return Response.json({ ok: true, item }, { status: 200, headers: NO_STORE });
  } catch (error) {
    if (error instanceof AttractionNotFoundError) {
      return Response.json(
        { ok: false, error: "Attraction not found." },
        { status: 404, headers: NO_STORE }
      );
    }
    console.error(`PUT /api/attractions/${id} failed:`, error);
    return Response.json(
      { ok: false, error: "Could not update the attraction." },
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error(`DELETE /api/attractions/${id} failed:`, error);
    return Response.json(
      { ok: false, error: "Could not delete the attraction." },
      { status: 500, headers: NO_STORE }
    );
  }
}
