...
import { reorderBranches } from "@/lib/branches/service";
...
export async function PUT(request: Request) {
  ...
  try {
    await reorderBranches(entries);
    return Response.json({ ok: true }, { status: 200, headers: NO_STORE });
  } catch (error) {
    console.error("PUT /api/branches/reorder failed:", error);
    return Response.json(
      { ok: false, error: "Could not reorder the branches." },
