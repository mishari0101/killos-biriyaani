...
import { reorderReviews } from "@/lib/reviews/service";
...
export async function PUT(request: Request) {
  ...
  try {
    await reorderReviews(entries);
    return Response.json({ ok: true }, { status: 200, headers: NO_STORE });
  } catch (error) {
    console.error("PUT /api/reviews/reorder failed:", error);
    return Response.json(
      { ok: false, error: "Could not reorder the reviews." },
